...
public:
	...
	FullyConnLayer(int num_neurons,int in_depth,int in_sx,int in_sy,FP l1_decay_mul=FP(0),FP l2_decay_mul=FP(1),FP bias_pref=FP(0)):in_depth(in_depth),in_sx(in_sx),in_sy(in_sy),l1_decay_mul(l1_decay_mul),l2_decay_mul(l2_decay_mul),layer_type("fc"),bias(bias_pref),in_act(NULL),out_act(NULL),biases(NULL),out_depth(num_neurons){
	cout << "cv 0" << endl;
